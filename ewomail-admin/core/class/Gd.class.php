...
class Gd{
    ...
    public function open($imgname){
        //检测图像文件
        if(!is_file($imgname)) E::error('不存在的图像文件');
    ...
    public function save($imgname, $type = null, $quality=80,$interlace = true){
        if(empty($this->img)) E::error('没有可以被保存的图像资源');
    ...
    public function width(){
        if(empty($this->img)) E::error('没有指定图像资源');
    ...
    public function crop($w, $h, $x = 0, $y = 0, $width = null, $height = null){
        if(empty($this->img)) E::error('没有可以被裁剪的图像资源');
    ...
    public function thumb($width, $height, $type = Image::IMAGE_THUMB_SCALE){
        if(empty($this->img)) E::error('没有可以被缩略的图像资源');
    ...
    public function water($source, $locate = Image::IMAGE_WATER_SOUTHEAST,$alpha=80){
        //资源检测
        if(empty($this->img)) E::error('没有可以被添加水印的图像资源');
        if(!is_file($source)) E::error('水印图像不存在');
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        ...
        //资源检测
        if(empty($this->img)) E::error('没有可以被写入文字的图像资源');
        if(!is_file($font)) E::error("不存在的字体文件：{$font}");
